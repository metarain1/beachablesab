...
if (isset($eltd_moose_options['lightbox_single_project'])) {
	$lightbox_single_project = $eltd_moose_options['lightbox_single_project'];
}
...
if (isset($eltd_moose_options['portfolio_gallery_image_hover_style'])) {
    $portfolio_gallery_hover_type = $eltd_moose_options['portfolio_gallery_image_hover_style'];
}

//gallery hover separator with text
if (isset($eltd_moose_options['portfolio_gallery_image_hover_separator'])) {
    $portfolio_gallery_image_hover_separator = $eltd_moose_options['portfolio_gallery_image_hover_separator'];
}
//is lightbox turned on for video single project?
if (isset($eltd_moose_options['lightbox_video_single_project'])) {
    $lightbox_video_single_project = $eltd_moose_options['lightbox_video_single_project'];
}
...
if (is_array($portfolio_images)){
	usort($portfolio_images, "eltdMooseComparePortfolioImages");
}
...
} elseif(isset($eltd_moose_options['portfolio_columns_number']) && $eltd_moose_options['portfolio_columns_number'] !== '') {
	$columns_number = 'v'.$eltd_moose_options['portfolio_columns_number'];
}
...
<div class="portfolio_gallery">
    ...
    if ($portfolio_m_images){
        ...
        foreach($portfolio_image_gallery_array as $gimg_id){
            ...
            $portfolio_gallery_thumb_size = 'eltd-moose-'.get_post_meta(get_the_ID(), 'eltd_choose-portfolio-image-size', true);
            $portfolio_gallery_thumb_size = (!empty($portfolio_gallery_thumb_size)) ? 'eltd-moose-'.get_post_meta(get_the_ID(), 'eltd_choose-portfolio-image-size', true) : 'full';
    ...
    if (is_array($portfolio_images) && count($portfolio_images)){
		foreach($portfolio_images as $portfolio_image){
			...
			<?php if($portfolio_image['portfolioimg'] != ""){ ?>
				...
				list($id, $title, $alt) = eltd_moose_get_portfolio_image_meta($portfolio_image['portfolioimg']);

                $single_image_id = eltd_moose_get_attachment_id_from_url($portfolio_image['portfolioimg']);
                if(!empty($single_image_id)){
                    $single_image_gallery_thumb_size = 'eltd-moose-'.get_post_meta(get_the_ID(), 'eltd_choose-portfolio-image-size', true);
                    $single_image_size = (!empty($single_image_gallery_thumb_size)) ? 'eltd-moose-'.get_post_meta(get_the_ID(), 'eltd_choose-portfolio-image-size', true) : 'full';
...
<div class="two_columns_75_25 clearfix portfolio_container">
    ...
    <div class="column2">
        <div class="column_inner">
            ...
            <h5 class="portfolio_detail_text_title"><?php _e("Project Details", "eltd") ?></h5>
                <?php
