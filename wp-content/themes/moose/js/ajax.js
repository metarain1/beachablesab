...
function ajaxSetActiveState(me){
    ...
    $j('.main_menu a, .mobile_menu a, .mobile_menu h4, .vertical_menu a, .popup_menu a, .side_menu a').parent().removeClass('active');
    ...
    $j('.main_menu a, .mobile_menu a, .vertical_menu a, .popup_menu a, .side_menu a').removeClass('current');
    me.addClass('current');
...
function callCallback(callbacks, name, self, args, variables) {
    ...
    eltdfAnimateOverlappingContent(); //needs to be in callback to wait all other functions to execute and than to fade in the content
}
	...
	if(hash !== undefined && $j('[data-eltd_id="'+hash+'"]').length > 0){
		...
		if($window_width > 1000){
			if($j('header.page_header').hasClass('fixed') && !$j('body').hasClass('vertical_menu_enabled')){
				...
				if(!$j('header.page_header').hasClass('transparent') || $j('header.page_header').hasClass('scrolled_not_transparent')){
					scrollToAmount = $j('[data-eltd_id="' + hash + '"]').offset().top -  header_height - large_menu_item_border - paspartuScrollAdd;
				}else{
					scrollToAmount = $j('[data-eltd_id="' + hash + '"]').offset().top - paspartuScrollAdd;
				}
...
function loadResource(url) {
    ...
    var me = $j("nav a[href='"+eltd_root+url+"'], .widget_nav_menu a[href='"+eltd_root+url+"']");
...
$j(document).ready(function() {
    ...
    //function is removed from call beacause eltd-menu.php does not have page_transitions conditions for active class,
    ...
    if($j('body').hasClass('woocommerce') || $j('body').hasClass('woocommerce-page')){
        ...
        $j(document).on('click','a[target!="_blank"]:not(.no_ajax):not(.no_link)',function(click){
            ...
            if($j(this).parent().parent().hasClass('flex-control-nav')){ return false; }
