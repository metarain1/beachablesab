...
if(version_compare(eltd_moose_get_vc_version(), '4.7.4') >= 0) {
	/**
	...
	if (($circle_hover == 'yes')) {
		$circle_hover_html .= '<div class="shader" style="background-color:' . $circle_color . ';">';
		$circle_hover_html .= '</div>';

		if ($hover_message != '') {

			$circle_hover_html .= '<h3 class="hover_message" style= "color:' . $message_color . ';">' . esc_attr($hover_message) . '</h3>';
...
// backward compatibility
	...
	$html = ( 'vc_box_shadow_3d' === $style ) ? '<span class="vc_box_shadow_3d_wrap">' . $img['thumbnail'] . '</span>' : $img['thumbnail'] . $circle_hover_html;
...
//to disable relative links uncomment this..
	...
	$rel = '';
	$rel_number = '';
	$img_output = ($style == 'vc_box_shadow_3d') ? '<span class="vc_box_shadow_3d_wrap">' . $img['thumbnail'] . '</span>' : $img['thumbnail'] . $circle_hover_html;
	if ($img_link_target !== "open_prettyphoto") {
		...
		$rel_number = rand();
		$rel = 'data-rel="prettyPhoto[rel-' . $rel_number . ']"';
