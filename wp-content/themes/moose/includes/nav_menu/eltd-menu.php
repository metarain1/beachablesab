...
if (!class_exists('eltd_type1_walker_nav_menu')) {
	...
	// add classes to ul sub-menus
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$wide_menu_background = isset($eltd_moose_options['enable_wide_manu_background']) && $eltd_moose_options['enable_wide_manu_background'] == 'yes';

			$indent = str_repeat("\t", $depth);
			if($depth == 0){
				$out_div = '<div class="second'.$background_image_class.'" '.($wide_menu_background ? eltd_moose_get_inline_style($menu_image_background) : '').'><div class="inner">';
			}else{
				$out_div = '';
			}
			
			// build html
			$output .= "\n" . $indent . $out_div  .'<ul '.(!$wide_menu_background ? eltd_moose_get_inline_style($menu_image_background) : '').'>' . "\n";
		}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$wide_background = '';
			if(isset($eltd_moose_options['enable_wide_manu_background']) && $eltd_moose_options['enable_wide_manu_background'] == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth == 0 && isset($eltd_moose_options['enable_dropdown_menu_item_icon']) && $eltd_moose_options['enable_dropdown_menu_item_icon'] == "yes" ){
				...
				if(isset($eltd_moose_options['menu_item_hover_animation']) && $eltd_moose_options['menu_item_hover_animation'] == "roll" && $depth == 0){
	...
	// add classes to ul sub-menus
		...
		// add main/sub classes to li's and links
		 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->description !== "") {
				$item_output .= '<span class="item_inner_description"> ' . $item->description;
                if($item->featured_icon !== ""){
                    $item_output .= '<span class=" featured_icon '. $item->featured_icon .'" aria-hidden="true"></span>';
                }
				$item_output .= '</span>';
...
if (!class_exists('eltd_type3_walker_nav_menu')) {
    class eltd_type3_walker_nav_menu extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            global $wp_query;
            global $eltd_moose_options_theme16;
            $sub = "";
            $indent = ( $depth > 0 ? str_repeat( "\t", $depth ) : '' ); // code indent
            if($depth >=0 && $args->has_children) :
                $sub = ' has_sub';
