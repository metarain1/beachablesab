 ...
 * It first checks global $eltd_moose_options variable and if option does'nt exists there
 * it checks default theme options
 *
 * @param $name string name of the option to retrieve
 * @return bool
 */
function eltdf_moose_option_has_value($name) {
	global $eltd_moose_options;
	global $eltdFramework;

	if (array_key_exists($name, $eltdFramework->eltdOptions->options)) {
		if(isset($eltd_moose_options[$name])) {
			return true;
		} else {
			return false;
		}
 ...
 * It first checks if option exists in $eltd_moose_options global array and if it does'nt exists there
 * it checks default theme options array.
 *
 * @param $name string name of the option to retrieve
 * @return mixed|void
 */
function eltdf_moose_option_get_value($name) {
	global $eltd_moose_options;
	global $eltdFramework;

	if (array_key_exists($name, $eltdFramework->eltdOptions->options)) {
		if(isset($eltd_moose_options[$name])){
			return $eltd_moose_options[$name];
		} else {
			return $eltdFramework->eltdOptions->getOption($name);
		}
	} else {
		global $post;
		...
		if (isset($value) && $value !== "") {
			...
			return $eltdFramework->eltdMetaBoxes->getOption($name);
		}
...
*@return bool|string
 *
 * @see eltd_moose_get_attachment_id_from_url()
 */
function eltdf_moose_get_attachment_thumb_url($attachment_url) {
	$attachment_id = eltd_moose_get_attachment_id_from_url($attachment_url);
 ...
 */
function eltd_moose_enqueue_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
	global $eltdFramework;

	$src = get_template_directory_uri().'/framework/admin/skins/'.$eltdFramework->getSkin().'/'.$src;
 ...
 */
function eltd_moose_register_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
	global $eltdFramework;
 ...
 */
function eltd_moose_enqueue_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false) {
	global $eltdFramework;
 ...
 */
function eltd_moose_register_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false) {
	global $eltdFramework;
...
if (!function_exists('eltd_moose_generate_dynamic_css_and_js')){
	/**
	 * Function that gets content of dynamic assets files and puts that in static ones
	 */
	function eltd_moose_generate_dynamic_css_and_js() {
		global $wp_filesystem;
		WP_Filesystem();

		$eltd_moose_options = get_option('eltd_options_moose');
		if(eltd_moose_is_css_folder_writable()) {
		...
		if(eltd_moose_is_js_folder_writable()) {
	...
	if(!is_multisite()) {
		add_action('eltd_after_theme_option_save', 'eltd_moose_generate_dynamic_css_and_js');
	}
}

if (!function_exists('eltd_moose_gallery_upload_get_images')) {
	/**
	 ...
	 */
	function eltd_moose_gallery_upload_get_images(){
		$ids=$_POST['ids'];
		$ids=explode(",",$ids);
		foreach($ids as $id):
	...
	add_action( 'wp_ajax_eltd_gallery_upload_get_images', 'eltd_moose_gallery_upload_get_images');
}

if (!function_exists('eltd_moose_hex2rgb')) {
	/**
	 ...
	 */
	function eltd_moose_hex2rgb($hex) {
		$hex = str_replace("#", "", $hex);
...
if(!function_exists('eltd_moose_get_attachment_meta')) {
	/**
	 ...
	 * @return array|mixed
	 */
	function eltd_moose_get_attachment_meta($attachment_id, $keys = array()) {
		$meta_data = array();
...
if(!function_exists('eltd_moose_get_attachment_id_from_url')) {
	/**
	 * Function that retrieves attachment id for passed attachment url
	 * @param $attachment_url
	 * @return null|string
	 */
	function eltd_moose_get_attachment_id_from_url($attachment_url) {
		global $wpdb;
		$attachment_id = '';
...
if(!function_exists('eltd_moose_get_attachment_meta_from_url')) {
	/**
...
*@return array|mixed
	 *
	 * @see eltd_moose_get_attachment_id_from_url()
	 * @see eltd_moose_get_attachment_meta()
	 *
	 * @version 0.1
	 */
	function eltd_moose_get_attachment_meta_from_url($attachment_url, $keys = array()) {
		...
		if(!empty($attachment_id)) {
			//get post meta
			$attachment_meta = eltd_moose_get_attachment_meta($attachment_id, $keys);
		}
...
if(!function_exists('eltd_moose_get_image_dimensions')) {
	/**
...
*@return array array of image sizes that containes height and width
	 *
	 * @see eltd_moose_get_attachment_meta_from_url()
	 * @uses getimagesize
	 *
	 * @version 0.1
	 */
	function eltd_moose_get_image_dimensions($url) {
		$image_sizes = array();

		//is url passed?
		if($url !== '') {
			//get image sizes from posts meta if attachment exists
			$image_sizes = eltd_moose_get_attachment_meta_from_url($url, array('width', 'height'));
...
if(!function_exists('eltd_moose_get_native_fonts_list')) {
	/**
	 * Function that returns array of native fonts
	 * @return array
	 */
	function eltd_moose_get_native_fonts_list(){
...
if(!function_exists('eltd_moose_get_native_fonts_array')) {
	/**
	 * Function that returns formatted array of native fonts
	 *
	 * @uses eltd_moose_get_native_fonts_list()
	 * @return array
	 */
	function eltd_moose_get_native_fonts_array(){
		$native_fonts_list = eltd_moose_get_native_fonts_list();
		$native_font_index = 0;
...
if(!function_exists('eltd_moose_is_native_font')) {
	/**
	 * Function that checks if given font is native font
	 * @param $font_family string
	 * @return bool
	 */
	function eltd_moose_is_native_font($font_family) {
		return  in_array(str_replace('+', ' ', $font_family), eltd_moose_get_native_fonts_list());
	}
}


if(!function_exists('eltd_moose_merge_fonts')) {
	/**
	 * Function that merge google and native fonts
	 *
	 * @uses eltd_moose_get_native_fonts_array()
	 * @return array
	 */
	function eltd_moose_merge_fonts() {
		global $eltd_moose_fonts_array;
		$native_fonts = eltd_moose_get_native_fonts_array();

		if(is_array($native_fonts) && count($native_fonts)){
			$eltd_moose_fonts_array = array_merge($native_fonts, $eltd_moose_fonts_array);
		}
	}

	add_action('admin_init', 'eltd_moose_merge_fonts');
}

if(!function_exists('eltd_moose_is_css_folder_writable')) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function eltd_moose_is_css_folder_writable() {
		$css_dir = get_template_directory().'/css';
...
if(!function_exists('eltd_moose_is_js_folder_writable')) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function eltd_moose_is_js_folder_writable() {
		$js_dir = get_template_directory().'/js';
...
if(!function_exists('eltd_moose_assets_folders_writable')) {
	/**
	 * Function that if css and js folders are writable
	 * @return bool
	 *
	 * @version 0.1
	 * @see eltd_moose_is_css_folder_writable()
	 * @see eltd_moose_is_js_folder_writable()
	 */
	function eltd_moose_assets_folders_writable() {
		return eltd_moose_is_css_folder_writable() && eltd_moose_is_js_folder_writable();
	}
}

if(!function_exists('eltd_moose_writable_assets_folders_notice')) {
	/**
	 ...
	 */
	function eltd_moose_writable_assets_folders_notice() {
		global $pagenow;

		$is_theme_options_page = isset($_GET['page']) && strstr($_GET['page'], 'eltd_moose_theme_menu');

		if($pagenow === 'admin.php' && $is_theme_options_page) {
			if(!eltd_moose_assets_folders_writable()) { ?>
				<div class="error">
					...
					For further instructions, please refer to our documentation.', 'eltd'); ?></p>
				</div>
			<?php }
		}
	}

	add_action('admin_notices', 'eltd_moose_writable_assets_folders_notice');
}

if(!function_exists('eltd_moose_inline_style')) {
	/**
	 * Function that echoes generated style attribute
	 *
*@param $value string | array attribute value
	 *
	 * @see eltd_moose_get_inline_style()
	 */
	function eltd_moose_inline_style($value) {
		echo eltd_moose_get_inline_style($value);
	}
}

if(!function_exists('eltd_moose_get_inline_style')) {
	/**
...
*@return string generated style attribute
	 ...
	 * @see eltd_moose_get_inline_style()
	 */
	function eltd_moose_get_inline_style($value) {
		return eltd_moose_get_inline_attr($value, 'style', ';');
	}
}

if(!function_exists('eltd_moose_class_attribute')) {
	/**
	 * Function that echoes class attribute
	 *
*@param $value string value of class attribute
	 *
	 * @see eltd_moose_get_class_attribute()
	 */
	function eltd_moose_class_attribute($value) {
		echo eltd_moose_get_class_attribute($value);
	}
}

if(!function_exists('eltd_moose_get_class_attribute')) {
	/**
...
*@return string generated class attribute
	 *
	 * @see eltd_moose_get_inline_attr()
	 */
	function eltd_moose_get_class_attribute($value) {
		return eltd_moose_get_inline_attr($value, 'class');
	}
}

if(!function_exists('eltd_moose_get_inline_attr')) {
	/**
	 ...
	 */
	function eltd_moose_get_inline_attr($value, $attr, $glue = '') {
		if(!empty($value)) {
...
if(!function_exists('eltd_moose_get_skin_uri')) {
    /**
     * Returns current skin URI
     * @return mixed
     */
    function eltd_moose_get_skin_uri() {
		global $eltdFramework;

		$current_skin = $eltdFramework->getSkin();

		return $current_skin->getSkinURI();
	}
}

if(!function_exists('eltd_moose_core_installed')) {
    /**
     * Checks if core plugin is installed
     * @return bool
     */
    function eltd_moose_core_installed() {
        return defined('ELATED_CORE_VERSION');
    }
}

if(!function_exists('eltd_moose_core_plugin_message')) {
    /**
     * Function that prints a mesasge in the admin if user hides TGMPA plugin activation message
     */
    function eltd_moose_core_plugin_message() {
        if(get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice', true) && !eltd_moose_core_installed()) {
            echo apply_filters('eltd_core_plugin_message', '<div class="update-nag">'.__('Installation of the "Elated
            Core" plugin is
            essential for
            proper
            theme functioning. Please <a href="'.admin_url('themes.php?page=install-required-plugins')
                    .'">install</a> this
            plugin and activate it', 'eltd').'</div>');
        }
    }

    add_action('admin_notices', 'eltd_moose_core_plugin_message');
}

if(!function_exists('eltd_moose_get_theme_info_item')) {
    /**
     * Returns desired info of the current theme
     * @param $item string info item to get
     * @return string
     */
    function eltd_moose_get_theme_info_item($item) {
        if($item !== '') {
...
if(!function_exists('eltd_moose_resize_image')) {
    /**
...
*@return array returns array containing img_url, width and height
     ...
     */
    function eltd_moose_resize_image($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true) {
        $return_array = array();

        //is attachment id empty?
        if(empty($attach_id) && $attach_url !== '') {
            //get attachment id from url
            $attach_id = eltd_moose_get_attachment_id_from_url($attach_url);
        }
...
if(!function_exists('eltd_moose_generate_thumbnail')) {
    /**
...
*@return string generated img tag
     *
     * @see eltd_moose_resize_image()
     * @see eltd_moose_get_attachment_id_from_url()
     */
    function eltd_moose_generate_thumbnail($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true) {
        //is attachment id empty?
        if(empty($attach_id)) {
        ...
        if(!empty($attach_id) || !empty($attach_url)) {
            $img_info = eltd_moose_resize_image($attach_id, $attach_url, $width, $height, $crop);
