...
class WpeCommon extends WpePlugin_common {
	...
	**/
    ...
    /**
     * The last-mod file is used by legacy staging to keep track of the staging site creation process
     * Gophpr processes do not have permission to access both live and legacy staging sites so last-mod
     * is kept in the live site's _wpeprivate dir. Otherwise last-mod can be found in the legacy staging
     * site's root dir.
     */
    public function last_mod_location() {
        $live_dir           = self::NAS_CONTENT . "/live/" . PWP_NAME;
        $live_touch_file    = "${live_dir}/". self::STAGING_STATUS_PATH_UNDER_LIVE;
        $staging_dir        = self::NAS_CONTENT . "/staging/" . PWP_NAME;
        $staging_touch_file = "${staging_dir}/". self::STAGING_STATUS_FILE;
        $have_staging_touchfile = is_dir( $staging_dir ) && file_exists( $staging_touch_file );
        return $have_staging_touchfile ? $staging_touch_file : $live_touch_file;
    }
    ...
    public function get_staging_status() {
        ...
        $staging_touch_file = $this->last_mod_location();
